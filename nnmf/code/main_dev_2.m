...
data_type = 'crossval'; % data set type; OPTIONS: 'crossval', 'sparse'

%% Load data
load('data/nnmf_best.mat') % Miro's 25 components
% load('data/nnmf_sparse_33.mat')
...
if strcmp(data_type, 'crossval')
    rank = 4; % number of components = k
    % cross-validated components (comment out if nec)
    load(['nnmf_cv_model_',num2str(rank),'.mat']); %8 cross-validated components
    A = Vt;
    Y = U;
    k_best = size(U,2);
end
...
if do_save
    ...
    elseif strcmp(data_type,'sparse')
        str = ['nnmf_sparse_' num2str(size(U,2))];    
    end
    hgexport(gcf,fullfile('figures', 'nnmf_num_components', str),hgexport('factorystyle'),'Format','eps');
    hgexport(gcf,fullfile('figures', 'nnmf_num_components', str))
%     close(gcf)
end
...
for ic = 1:length(cells)
    ...
    plot(psth.ts,X(:,cells(ic)), 'color', 'k')
...
%     set(gca,'XTick',1:1:17,'XTickLabel',{fliplr(OUTPERM)})
    ...
    subplot(2,5,10)
    plot(fliplr(sort(Y_norm(cells(ic),:))), 'k')
    ...
    if do_save
        % save figure
        if strcmp(data_type,'crossval')
            str = ['nnmf_cv_cell_recon_' num2str(cells(ic))];
        elseif strcmp(data_type,'sparse')
            str = ['nnmf_sparse_cell_recon_' num2str(cells(ic))];    
        end
        hgexport(gcf,fullfile('figures', 'cell_reconstructions', str),hgexport('factorystyle'),'Format','eps');
        hgexport(gcf,fullfile('figures', 'nnmf_num_components', str))
    %     close(gcf)
    end
end
...
%%
plot(fliplr(sort(Y_norm(cells(ic),:))))
