%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NNMF Analysis - Development
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INFO: This script needs some serious clean-up, outsourcing all the
% functions and then calling only what's necessary, as most of the analyses
% here did not go into the final figures. This is why this was renamed 'main
% > main_dev', and returns in the middle of the script. Needs to be amended
% in future. The functions themselves are badly organized, and their inputs
% and outputs could be sparser. Often, analyses and plotting are mixed up
% here and should be separated.

clear all; 
ranks = 29; % Select NNMF models of various ranks to plot
for irank = ranks
    close all;
    %% Parameters
    ...
    % Set plots to make
    do_plot_nnmf_components = true;
    do_plot_nnmf_example_cells = false;
    do_corr_nnmf_rgc = true;
    ...
    %% Plot NNMF components (dendrogram, components, %cells)

    if do_plot_nnmf_components
        [fig, axes, OUTPERM] = plot_nnmf_components(A, psth, Y_norm);

        % Save fig
    %     save_fig = false;
        if save_fig
            % save figure
            fname = ['nnmf_', data_type, '_', num2str(size(A,1))];        
    ...
    %% Plot example cell responses and reconstructions from NNMF components

    % Pars
    cell_idx = [226 507 537];
    if do_plot_nnmf_example_cells
        % Pars
        save_fig = true;
        plot_nnmf_example_cells(cell_idx, psth, A, X, Y, Y_norm, OUTPERM, save_fig, data_type);
    end
    ...
    if do_corr_nnmf_rgc
        ...
        [max_corrs, fig] = corr_nnmf_rgc(OUTPERM, psth, A);

        % Pars
    %     save_fig = false;
          if save_fig
            fname = ['nnmf_', data_type, '_', num2str(size(A,1))];
            ffname = fullfile(results_dir, 'corr_nnmf_rgc', fname);
            fprintf('Saving figure to %s ...\n', ffname);
    %         hgexport(gcf, ffname, hgexport('factorystyle'),'Format','eps');
            saveas(gcf, ffname, 'epsc');
            saveas(gcf, ffname, 'png');
            hgexport(gcf, ffname);        
        end
    end
...
%% COMPUTE FEATURE CORRELATIONS MATRIX
...
return
%% Check MATLAB's sparse-NNMF results
[Y,A,~,~,D_r] = sparsenmfnnls(psth.psth,33);
A
Y
psth

save('data/nnmf_sparse_33.mat', 'A', 'Y', 'psth')
